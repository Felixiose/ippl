...
namespace CatalystAdaptor {
    ...
    void Initialize_Adios(int argc, char* argv[]) {
        std::cout << "Starting Initialize_Adios..." << std::endl;

        conduit_cpp::Node node;
        for (int cc = 1; cc < argc; ++cc) {
            if (strstr(argv[cc], "xml")) {
                node["adios/config_filepath"].set_string(argv[cc]);
                std::cout << "Set adios/config_filepath to " << argv[cc] << std::endl;
            } else {
                std::string script_key = "catalyst/scripts/script" + std::to_string(cc - 1);
                node[script_key].set_string(argv[cc]);
                std::cout << "Set " << script_key << " to " << argv[cc] << std::endl;
            }
        }

        const char* impl_name  = getenv("CATALYST_IMPLEMENTATION_NAME");
        const char* impl_paths = getenv("CATALYST_IMPLEMENTATION_PATHS");

        if (impl_name && impl_paths) {
            node["catalyst_load/implementation"]        = impl_name;
            node["catalyst_load/search_paths/paraview"] = impl_paths;
            std::cout << "Set catalyst_load/implementation to " << impl_name << std::endl;
            std::cout << "Set catalyst_load/search_paths/paraview to " << impl_paths << std::endl;
        } else {
            std::cerr << "Error: Environment variables CATALYST_IMPLEMENTATION_NAME and/or "
                         "CATALYST_IMPLEMENTATION_PATHS are not set."
                      << std::endl;
            exit(EXIT_FAILURE);
        }
        ...
        std::cout << "Catalyst initialized successfully." << std::endl;
    }

    template <class Field>
    std::optional<conduit_cpp::Node> Execute_Field(int cycle, double time, int rank, Field& field,
        ...
        for (unsigned int iDim = 0; iDim < field.get_mesh().getGridsize().dim; ++iDim) {
            ...
            // add origin
            mesh[field_node_origin].set(field.get_mesh().getOrigin()[iDim]
    ...
    std::optional<conduit_cpp::Node> Execute_Particle(int cycle, double time, int rank,
        ...
        mesh["coordsets/coords/values/x"].set_external(
            &layout_view.data()[0][0], particleContainer->getLocalNum(), 0, sizeof(double) * 3);
        mesh["coordsets/coords/values/y"].set_external(
            &layout_view.data()[0][1], particleContainer->getLocalNum(), 0, sizeof(double) * 3);
        mesh["coordsets/coords/values/z"].set_external(
            &layout_view.data()[0][2], particleContainer->getLocalNum(), 0, sizeof(double) * 3);
        ...
        mesh["topologies/mesh/elements/connectivity"].set_external(
            particleContainer->ID.getView().data(), particleContainer->getLocalNum());
        ...
        fields["charge/values"].set_external(particleContainer->q.getView().data(),
        ...
        fields["velocity/values/x"].set_external(
            &velocity_view.data()[0][0], particleContainer->getLocalNum(), 0, sizeof(double) * 3);
        fields["velocity/values/y"].set_external(
            &velocity_view.data()[0][1], particleContainer->getLocalNum(), 0, sizeof(double) * 3);
        fields["velocity/values/z"].set_external(
            &velocity_view.data()[0][2], particleContainer->getLocalNum(), 0, sizeof(double) * 3);
        ...
        fields["position/values/x"].set_external(
            &layout_view.data()[0][0], particleContainer->getLocalNum(), 0, sizeof(double) * 3);
        fields["position/values/y"].set_external(
            &layout_view.data()[0][1], particleContainer->getLocalNum(), 0, sizeof(double) * 3);
        fields["position/values/z"].set_external(
            &layout_view.data()[0][2], particleContainer->getLocalNum(), 0, sizeof(double) * 3);
    ...
    void Execute_Field_Particle(int cycle, double time, int rank, Field& field,
